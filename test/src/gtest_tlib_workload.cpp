...
inline auto check_index_space_division(size_type init, size_type steps)
{
	...
	for(auto const& n : shapes)
	{
		...
		for(auto const& l : layouts)
		{
			...
			auto w = tlib::ttv::detail::generate_strides(n,l);
			...
			for(auto m = 1u; m <= rank; ++m)
			{
				...
				auto layouts = tlib::ttv::detail::divide_layout(l.data(), rank, m); // (l1,l2) <- divide(l) based on the contraction m.
				const auto l1 = layouts.first;
				const auto l2 = layouts.second;

				auto strides = tlib::ttv::detail::divide(w.data(), l.data(), rank, m); // (w1,w2) <- divide(w) based on permutation l and contraction m.
				const auto w1 = strides.first;
				const auto w2 = strides.second;

				auto shapess = tlib::ttv::detail::divide(n.data(), l.data(), rank, m); // (n1,n2) <- divide(n) based on permutation l and contraction m.
				const auto n1 = shapess.first;
				const auto n2 = shapess.second;

				auto v1 = tlib::ttv::detail::generate_strides(n1,l1); // compute new strides based on n1 and l1
				auto v2 = tlib::ttv::detail::generate_strides(n2,l2);
				
				auto nprod = std::accumulate(n.begin(), n.end(), 1ul, std::multiplies<std::size_t>());

				for( auto j = 0u; j < nprod; ++j) {

					auto i = tlib::ttv::detail::at_1( j, w, l ); // compute i
					EXPECT_EQ ( tlib::ttv::detail::at( i, w ) , j );

					auto iis = tlib::ttv::detail::divide(i.data(),l.data(), nn ,m); // (i1,i2) <- divide(i) based on permutation l and contraction m.
					auto i1 = iis.first;
					auto i2 = iis.second;

					auto j1 = tlib::ttv::detail::at(i1,w1); // j1=dot(i1,w1)
					auto j2 = tlib::ttv::detail::at(i2,w2); // j2=dot(i2,w2)

					EXPECT_EQ ( j1+j2 , j ); // make sure that the division of w and i did not mess up sth.

					auto k1 = tlib::ttv::detail::at(i1,v1); // k1=dot(i1,v1)
					auto k2 = tlib::ttv::detail::at(i2,v2); // k2=dot(i2,v2)

					//auto ii1 = multi_index_t(v1.size());
					//auto ii2 = multi_index_t(v2.size());

					auto ii1 = tlib::ttv::detail::at_1(k1, v1, l1); // compute ii1 which must be i1 as the v1 is extracted from k1
					auto ii2 = tlib::ttv::detail::at_1(k2, v2, l2); // compute ii2 which must be i2 as the v2 is extracted from k2
					...
					auto jj1 = tlib::ttv::detail::at(ii1,w1);
					auto jj2 = tlib::ttv::detail::at(ii2,w2);
					...
					auto jjj1 = tlib::ttv::detail::at_at_1(k1, v1, w1, l1);
					auto jjj2 = tlib::ttv::detail::at_at_1(k2, v2, w2, l2);
